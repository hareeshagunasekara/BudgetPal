...
import com.example.budgetpal.budgetpal.model.Transaction
import com.example.budgetpal.budgetpal.model.TransactionType
...
class PreferenceManager(context: Context) {
    ...
    companion object {
        private const val PREF_NAME = "BudgetpalPrefs"
    ...
    fun createBackup(backupData: String): Boolean {
        try {
            ...
            val fileName = "BudgetPal_Backup_${dateFormat.format(Date())}.json"
            val backupDir = File(context.filesDir, "backups")
            if (!backupDir.exists()) {
                backupDir.mkdirs()
            }
